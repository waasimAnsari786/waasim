...
const ContactForm = () => {
  ...
  const handleOnSubmit = (e) => {
    ...
    if (!validateUserName(InpObj.name)) {
      alert('Your username is incorrect. Please enter a valid username with only letters and at least 3 characters.');
      return; // Stop form submission if validation fails
    }

    if (!validatePhoneNum(InpObj.number)) {
      alert('Your phone number is incorrect. Please enter a valid Pakistani phone number in the format: 03XX-XXXXXXX or +923XX-XXXXXXX.');
      return; // Stop form submission if validation fails
    }
    ...
    emailjs
      ...
      .catch((err) => {
        // Error alert message
        alert('Maybe my submission has expired. You can contact me on my LinkedIn or email: waasimansari92@gmail.com');
      });
  };
        ...
        <div className="w-full md:w-2/3">
          <form className="w-full rounded-lg flex flex-col gap-2">
            <div className="flex flex-col md:flex-row gap-2">
              <Input inpClass='w-full md:w-1/2' inpId='name' inpPlaceHolder='Your Name' inpFunc={handleOnChange} inpVal={InpObj.name} />
              <Input inpClass='w-full md:w-1/2' inpId='number' inpPlaceHolder='Your Phone Number' inpFunc={handleOnChange} inpVal={InpObj.number} />
            </div>
            <div>
              <Input inpClass='w-full' inpId='email' inpPlaceHolder='Your Email' inpFunc={handleOnChange} inpVal={InpObj.email} />
            </div>
